...
#### Ernesto Acosta: Calculational HoTT
...
#### Steve Awodey: A Quillen model structure on the category of cartesian cubical sets
...
#### Bruno Bentzen: Informal cubical type theory
...
#### Guillaume Brunerie: A formalization of the initiality conjecture in Agda
...
#### Vikraman Choudhury: The finite-multiset construction in HoTT
...
#### Dan Christensen: Localization at a prime in homotopy type theory
...
Real-cohesive homotopy type theory is an extension of homotopy type theory obtained by adding a string of modalities $ \int \dashv \flat \dashv \sharp $ to encode Lawvere's axiomatic cohesion.  The $ \int $ modality compares higher inductive types with their set-theoretic counterparts, e.g. the circle as a 0-type $ \mathtt{base} $ and 1-type $ \mathtt{loop} \colon \mathtt{base} \to \mathtt{base} $ and the circle as a set $ \{ (x,y) \vert x,y \in \mathbb{R} and x^2+y^2=1 \} $.  The $ \flat $ modality allows for non-continuous constructions, such as choosing the fixed point in the proof of Brower's fixed point theorem.  In this talk, we extend the use-case for real-cohesive homotopy type theory by importing a HoTT version of the classic Borsuk-Ulam theorem.  
...
#### James Francese: Toward a Homotopy Tripos for Higher Realizability
...
#### Nicola Gambino: Towards a constructive simplicial model of univalent foundations
...
First we describe an essentially algebraic theory,  $\Theta_{\Z}$, for Z-categories, a notion of higher category wherein all entities are morphisms between lower morphisms. We show that the model category induced by the test model structure on cellular sets induces a model structure on presheaves on $\Theta_{\Z}$ which presents weak Z-groupoids. We will then see that subcategory of pointed objects in this presheaf topos satisfying a finiteness condition present the (\infty,1) category of spectra.

We will then relate the stable homotopy hypothesis that puts connective spectra as equivalent to picard \infty-groupoids to this presentation of spectra by observing that pointed Z-categories come equipped with a canonical monoidal structure.

We will observe that this structure is an avatar of one the equivalent definitions of monoidal (\infty,n) category, and sketch how any stable (\infty,1)-category should be presentable as a pointed (Z,1)-category.
...
#### Giulio Lo Monaco: Set-theoretic remarks on a possible definition of elementary oo-topos

In axiomatizing the significance of oo-toposes in homotopy type theory, one realizes that dependent products and sums should be required to be recoverable within suitable classes of morphisms, namely some classes for which there exists a classifier. However, asking this of dependent products turns out to be a quite strong request. In fact, once we fix a Grothendieck universe, the possibility of finding classes of morphisms in an oo-topos which both have a classifier and are closed under dependent products will be proven to be equivalent to the universe being 1-inaccessible, a condition which is strictly stronger than merely being inaccessible.

#### Andreas Lynge: Universal Algebra in HoTT
...
#### Martin Bidlingmaier: Coherence via big categories with families of locally cartesian closed categories
...
#### Anders M&ouml;rtberg: A Unifying Cartesian Cubical Set Model
...
#### David Jaz Myers: Looking at Functions through the Modal Prism
...
We will then see how these notions can be used to do algebraic topology in Real Cohesive HoTT. As an example, we will calculate the homotopy type of the topological circle $\{(x, y) \in \mathbb{R}^2 \mid x^2 + y^2 = 1\}$ without first expressing it as a higher inductive type.
...
#### Robert Rose: Constructing the natural numbers in intensional type theories with S1
...
#### Christian Sattler: Homotopy canonicity
...
#### Matthieu Sozeau: Equations for HoTT
...
[2] Rasekh - Complete Segal objects  [3] Shulman - Univalence for inverse diagrams and homotopy canonicity

#### Jonathan Sterling: XTT: Cubical Exact Equality and Categorical Gluing
...
An extended version of our paper is available at: https://arxiv.org/abs/1904.08562

#### Chaitanya Leena Subramaniam: Contextual categories as monoids in a category of collections
...
#### Andrew Swan: On Church's Thesis in Cubical Assemblies
...
#### Taichi Uemura: A General Framework for the Semantics of Type Theory
...
#### Matthew Z. Weaver: A Model of Type Theory with Directed Univalence in Bicubical Sets
...
#### Colin Zwanziger: Natural Model Semantics of Comonadic Modal Type Theory
